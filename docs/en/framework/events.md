...
Harness Cordis events use `namespace/action` names, including `agent/step`, `agent/request`, `agent/request-error`, `tools/result`, and `session/event`. The generated `cordis-surface` regions on the [subsystem pages](https://github.com/deepseek-ai/DeepSeek-Harness/blob/master/docs/subsystems/core.md) record complete signatures and modes.
