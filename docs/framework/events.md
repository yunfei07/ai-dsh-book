...
[English](/en/framework/events) | 中文
...
Harness 的 Cordis 事件遵循 `namespace/action` 命名，例如 `agent/step`、`agent/request`、`agent/request-error`、`tools/result` 和 `session/event`。完整签名与触发模式见[子系统页面](https://github.com/deepseek-ai/DeepSeek-Harness/blob/master/docs/subsystems/core.md)上生成的 `cordis-surface` 区块。
